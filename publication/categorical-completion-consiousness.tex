...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\textwidth]{figures/olfactory_spatial_analysis.png}
\caption{\textbf{Spatial distribution of oxygen molecules reveals ensemble structure in H$^+$ substrate.} 
\textbf{(A)} 3D oxygen distribution: 50 BMDs (spheres) distributed in 60\times60\times40 angstrom volume. Color indicates distance from center of mass (COM): purple (5 angstrom, near center) to yellow (30 angstrom, periphery). Spheres cluster centrally with sparse periphery, indicating oxygen molecules aggregate within H$^+$ electric field. Grid shows coordinate system (X, Y, Z in angstroms). Central clustering suggests field-mediated attraction—oxygen molecules drawn to regions of optimal electric field configuration. 
\textbf{(B)} XY projection: Top-down view reveals radial symmetry. 50 BMDs (circles) color-coded by distance (purple=5 angstrom, yellow=30 angstrom). Dense central cluster (±10 angstrom) with sparse outer ring (±30 angstrom). Circular distribution confirms isotropic field geometry—no preferred direction, consistent with uniform H$^+$ flux. This symmetry simplifies analysis: consciousness mechanisms are rotationally invariant. 
\textbf{(C)} Radial distribution from COM: Histogram shows distance distribution (x-axis: 5-30 angstrom). Peak at 15 angstrom (16 counts) with mean=16.0 angstrom (red dashed line) and median=15.7 angstrom (orange dashed line). Near-Gaussian distribution indicates thermodynamic equilibrium—oxygen molecules settle at characteristic distance determined by balance between H$^+$ field attraction and thermal motion. Standard deviation \sim5 angstrom indicates moderate spatial spread. 
\textbf{(D)} Coordinate distributions: Stacked histogram shows X (red), Y (green), and Z (blue) position distributions. All three centered at origin (0 angstrom) with similar widths (±20 angstrom), confirming isotropic distribution. Peak at 0 angstrom (30-35 counts combined) indicates central concentration. Symmetric tails extend to ±30 angstrom. Equal X, Y, Z distributions prove no spatial bias—oxygen ensemble is spherically symmetric within H$^+$ substrate, validating field-based (not geometry-based) organization. This establishes that consciousness substrate is defined by electric field topology, not anatomical structure.}
\end{figure}
...
Traditional: "Enzymes aren't consumed because they're released unchanged after the reaction"
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\textwidth]{figures/molecular_signatures_analysis.png}
\caption{\textbf{Molecular oscillatory signatures reveal weak force diversity underlying qualia.} 
\textbf{(A)} Molecular property comparison (oscillatory frequency in Hz, $\times$10$^{13}$): Five odorant molecules show similar mean frequencies (6.3$\times$10$^{13}$ Hz, annotated in box: mean=6.30$\times$10$^{13}$, std=3.33$\times$10$^{12}$, range=5.93-6.92$\times$10$^{13}$ Hz). Vanillin (vanilla, blue bar, 6.14$\times$10$^{13}$ Hz), ethyl vanillin (vanilla, red, 6.93$\times$10$^{13}$ Hz), benzene (aromatic, green, 5.93$\times$10$^{13}$ Hz), indole (fecal, orange, 5.93$\times$10$^{13}$ Hz), and citral (lemon, purple, 6.92$\times$10$^{13}$ Hz). Despite similar frequencies, molecules produce distinct scents—demonstrating that qualia arise from weak force configurations (Van der Waals angles, dipole orientations), not fundamental frequencies alone. Numerical annotations show exact values for each molecule. 
\textbf{(B)} Signature space (PCA): Principal component analysis reveals 100\% variance explained by PC1 (x-axis, $\times$10$^{12}$) and PC2 (y-axis, 0.0\%). Molecules cluster in distinct regions: Vanillin (large blue circle, PC1$\sim$-2), ethyl vanillin (red circle, PC1$\sim$-3, PC2$\sim$20), benzene (green circle, PC1$\sim$0, PC2$\sim$-50), indole (orange circle, PC1$\sim$-3, PC2$\sim$-5), citral (purple circle, PC1$\sim$6, PC2$\sim$0). Spatial separation confirms unique oscillatory signatures despite similar base frequencies. Annotations label each molecule and indicate total variance explained (100.0\%, yellow box). Circle size proportional to molecular weight. This establishes that weak force arrangements create distinguishable oscillatory patterns—the physical basis of different qualia from similar molecules.}
\label{fig:molecular_signatures}
\end{figure}


\textbf{Example 1 - Enzymes}: An enzyme active site (BMD) accepts multiple substrates with similar shapes (categorical equivalence at spatial level) but different chemical properties. Which substratees information about the cethe cellulartate.
...
\item Drastic increase in probability for allowed completions
...
(2) \textbf{Multiple completions}: From phase-lock degeneracy, $|\Omega_{\uparrow}| \sim 10^6$ to $10^{11}$, physically distinct arrangements produce the same $\Omega_{required}$.
...
(4) \textbf{Physical implementation}: Holes are physical absences in oscillatory cascades, completed by neurones generating real oscillatory patterns.

Therefore, each oscillatory hole \textit{is} a BMD. The filtering function is the hole's requirement $\Omega_{required}$. The term "information catalysis" refers to the selection of one completion from millions of possibilities. The "biological" aspect is that this process enables cellular reactions to proceed at biological speeds, bypassing diffusion limits.
$\square$
\end{proof}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\textwidth]{figures/olfactory_geometry_analysis.png}
\caption{\textbf{Oscillatory hole geometry in olfactory ensemble reveals circuit completion structure.} 
\textbf{(A)} Hole radius distribution across 50 biological Maxwell demons (BMDs): Histogram shows peak at 3.5 \text{\AA} (red dashed line indicates mean=3.46 Å). Distribution spans 2.0-5.0 \text{\AA} with mode at 2.5 \text{\AA} (7 counts). Color gradient (purple to yellow) indicates density. This characteristic size matches Van der Waals radii of small molecules, confirming holes are molecular-scale oscillatory gaps requiring specific completion patterns. 
\textbf{(B)} Density vs. radius correlation: Strong negative correlation (r=-0.881, p=3.01$\times$10$^{-17}$, annotated in yellow box). Molecular density (y-axis, 0.00-0.14) decreases exponentially with hole radius (x-axis, 2.0-5.0 \text{\AA}). Red dashed line shows linear fit. Individual points color-coded by density (orange=high, purple=low). Physical interpretation: Larger holes require more precise oscillatory patterns (lower density of compatible configurations), making completion more selective. This selectivity is the physical basis of specificity in perception and thought. 
\textbf{(C)} Hole center positions in 3D space: Spheres represent 50 BMD holes distributed within 40$\times$40$\times$40 \text{\AA} volume. Color indicates radius (blue=2.5 \text{\AA}, red=4.5 \text{\AA}). Spatial clustering visible near origin, suggesting preferred locations in molecular ensemble. Grid shows coordinate system (X, Y, Z in Ångströms). Non-random distribution indicates geometric constraints on hole formation—holes emerge at specific locations determined by molecular arrangement. 
\textbf{(D)} Geometry evolution over 50 BMD configurations: Dual y-axis plot shows hole radius (blue circles, left axis, 2.0-5.0 \text{\AA}) and molecular density (red squares, right axis, 0.00-0.14) vs. geometry index (x-axis, 0-50). Radius fluctuates with period $\sim$10 indices, while density anti-correlates (peaks when radius minimizes). This temporal structure reveals that hole geometry oscillates—holes open and close rhythmically, requiring continuous neural completion. Consciousness is not static perception but continuous circuit re-completion at $\sim$40 Hz (matching gamma oscillations).}
...
\caption{\textbf{Neural oscillations reveal multi-frequency coupling underlying consciousness.} 
\textbf{(A)} Raw EEG signal over 10 seconds shows complex multi-frequency oscillations (amplitude range: -100 to +100 $\mu$V). Periodic structure indicates nested oscillatory hierarchy. 
\textbf{(B)} Power spectral density (PSD) on logarithmic scale reveals dominant peaks at delta (1-4 Hz), theta (4-8 Hz), alpha (8-13 Hz), beta (13-30 Hz), and gamma (30-100 Hz) bands. Peak power at $\sim$10$^3$ $\mu$V$^2$/Hz in theta band indicates strong low-frequency modulation. 
\textbf{(C)} Brain wave band power distribution: Delta dominates at 42.8\%, followed by gamma (21.9\%), theta (15.5\%), alpha (13.3\%), beta (5.7\%), and high gamma (0.3\%). Delta dominance suggests deep processing or sleep-like state. Color coding: delta (purple), theta (dark blue), alpha (teal), beta (green), gamma (yellow), high gamma (cyan). 
\textbf{(D)} Frequency components decomposed over 5 seconds: Delta (blue, 0-50 $\mu$V), theta (orange, 50-100 $\mu$V), alpha (green, 100-150 $\mu$V), and beta (red, 150-200 $\mu$V) oscillate at distinct frequencies with phase relationships. Vertical stacking reveals hierarchical amplitude modulation. 
\textbf{(E)} Cross-frequency coupling analysis: Gamma coherence shows strongest coupling (0.053), followed by delta-theta coupling (0.016), alpha-beta coupling (0.012), and theta-gamma phase-amplitude coupling (PAC, 0.011). Low coupling values indicate weak cross-frequency interactions. 
\textbf{(F)} Gamma oscillations (30-100 Hz) over 2 seconds exhibit high-frequency, low-amplitude structure (±30 $\mu$V) characteristic of local cortical processing. Rapid fluctuations indicate fast neural communication. 
\textbf{(G)} Alpha-beta interaction: Alpha envelope (blue, 10-30 $\mu$V) and beta envelope (orange, 35-45 $\mu$V) show anti-phase relationship over 10 seconds, suggesting competitive dynamics between attention (alpha) and motor planning (beta) systems. 
\textbf{(H)} Theta-gamma phase-amplitude coupling (PAC): Histogram shows gamma amplitude modulation by theta phase (MI=0.012). Peak at -1 radian indicates gamma bursts occur at specific theta phases, supporting nested oscillation theory. 
\textbf{(I)} Validation summary (red box): Status FAIL. Alpha dominance (13.3\%) below expected range (20-40\%), indicating reduced attentional engagement. Theta-gamma PAC (0.012) below threshold (≥0.1), suggesting weak cross-frequency coupling. Alpha-beta coupling (0.011) outside expected range (-0.7 to -0.2), indicating abnormal competitive dynamics. These metrics suggest sub-optimal conscious processing state.}
...
\item Cannot explain memory generalisation (applying past experiences to novel situations)
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\textwidth]{figures/thought_comparison_analysis.png}
\caption{\textbf{Thought library analysis reveals geometric structure of completed circuits.} 
\textbf{(A)} Energy comparison across four thoughts: All thoughts exhibit near-zero energy (y-axis: -0.04 to +0.04, arbitrary units). Thought 0 (red bar, x=0) shows minimal positive energy. Thoughts 1-3 (black bars) at exactly zero. Annotation box: "Min=-0.000e+00, Max=-0.000e+00, Mean=0.000e+00" confirms negligible energy differences. Physical interpretation: Completed circuits have equivalent energies—thought content is not encoded in energy levels but in oscillatory signatures (weak force configurations). This explains why thinking requires no more metabolic effort than baseline (Section 9.6): all thoughts are energetically equivalent circuit completions. 
\textbf{(B)} Signature heatmap: 4 thoughts (rows, y-axis: 0-3) $\times$ 30 signature components (columns, x-axis: 0-30). Color indicates coupling strength (dark blue=0.0, red=0.8). Thought 0 shows strong coupling in components 0-5 (light blue, $\sim$0.4) and 20-25 (orange-red, $\sim$0.7). Thoughts 1-3 show distinct patterns: Thought 1 peaks at components 10-15, Thought 2 at 15-20, Thought 3 at 25-30. Each thought has unique signature despite identical energy. This is the physical basis of qualia: different weak force arrangements (signature components) create different subjective experiences from the same oscillatory hole. 
\textbf{(C)} Signature space (PCA): Two principal components explain 86.7\% variance (yellow box). PC1 (x-axis, 61.9\%) separates thoughts 0, 1, 2, 3 (labeled circles) along horizontal axis. PC2 (y-axis, 24.8\%) provides secondary separation. Thought 0 (circle at PC1$\sim$-0.3, PC2$\sim$-0.05) isolated from thoughts 1-3 (clustered at PC1$\sim$0.1). Color indicates energy (purple=-0.1, yellow=+0.1). Spatial separation confirms thoughts are geometrically distinct in signature space despite identical energies—consciousness discriminates configurations, not energies. 
\textbf{(D)} Spatial statistics: Mean distance (blue bars) and std distance (red bars) for each thought. All thoughts show distance $\sim$0.09 Å (y-axis: 0.00-0.10 Å). Thought 0 has slightly higher std (red bar $\sim$0.03 Å) indicating more variability. Annotation: "All thoughts: N=51 O" indicates 51 oxygen molecules analyzed per thought. Consistent distances across thoughts suggest characteristic length scale $\sim$0.1 Å for circuit completion—this is the precision required for weak force matching. Sub-Ångström precision explains why thoughts are so specific: tiny geometric changes create entirely different experiences.}
...
\textbf{Result}: All cell types cluster around 0.5\%, confirming the theoretical prediction within experimental uncertainty. $\checkmark$

\subsection{Membrane-O$_2$ Phase-Locking}

\textbf{Prediction}: The membrane electron cascade frequency should match O$_2$ electron pair oscillation frequency ($\sim$10$^{13}$ Hz) when phase-locked.
...
\caption{\textbf{Cognitive state dynamics reveal hierarchical processing architecture.} 
\textbf{(A)} Cognitive state levels over 175 seconds: Attention (purple, oscillating 2-3), working memory (blue, stable $\sim$1), executive function (red, minimal $<$1), and consciousness (green, periodic peaks to 7). Attention shows regular 50-second cycles, suggesting rhythmic engagement. Consciousness peaks coincide with attention maxima, indicating coupling. 
\textbf{(B)} Neural oscillations across four frequency bands over 10 seconds: Working memory (blue, 0-50 offset), attention (green, 50-100 offset), executive (red, 100-150 offset), and consciousness (purple, 150-200 offset). High-frequency components visible in all bands. Consciousness band shows highest amplitude variability, consistent with integration role. 
\textbf{(C)} Cognitive performance over time: Reaction time (red, 350-420 ms, left axis) and accuracy (blue, 0.6-1.3, right axis) show coupled oscillations with $\sim$50-second period matching attention cycles. Anti-phase relationship: accuracy peaks when reaction time minimizes, indicating speed-accuracy tradeoff. 
\textbf{(D)} Cognitive network coupling matrix: Attention shows strong self-coupling (dark red diagonal, 1.0) but weak coupling to other systems (dark blue off-diagonal, $<$0.2). Working memory, executive, and consciousness similarly isolated. Low inter-system coupling (all $<$0.4) indicates functional segregation rather than integration. 
\textbf{(E)} Key coupling relationships: Attention-executive coupling (0.004, cyan), working memory-consciousness coupling (0.004, green), and cognitive coherence (0.003, yellow) all extremely weak. These values orders of magnitude below functional thresholds. 
\textbf{(F)} RT-neural correlation scatter plot (r=0.329): Reaction time (360-420 ms) shows weak positive correlation with attention neural activity (0-20 arbitrary units). Red points (n$\sim$1000) with positive trend line (dashed red) indicate slower responses with higher neural activity—counterintuitive result suggesting inefficient processing. 
\textbf{(G)} Processing efficiency over time: Orange curve oscillates between 0.3-0.8 with $\sim$50-second period. Yellow shading indicates baseline (0.3). Peaks align with attention maxima, confirming efficiency depends on attentional state. 
\textbf{(H)} Cognitive resources over time: Red curve oscillates between 0.25-1.75 with same 50-second period. Resource availability drives attention cycles, suggesting metabolic or oscillatory constraint. 
\textbf{(I)} Validation summary (red box): Status FAIL. Attention-executive coupling (0.004) far below required threshold (≥0.4). WM-consciousness coupling (0.004) below threshold (≥0.35). RT-neural correlation (0.329) outside expected range (-0.8 to -0.2), indicating abnormal processing. Cognitive coherence (0.003) below threshold (≥0.3). Multiple failures indicate disrupted cognitive architecture inconsistent with healthy conscious processing.}
...
\textbf{Prediction}: The conscious frame rate should match the oscillatory hole generation rate, approximately 30-50 Hz, during normal waking consciousness.

\textbf{Measurement}: EEG gamma-band coherence (30-80 Hz) as a proxy for BMD generation:
\begin{itemize}
...
\item Anaesthesia: $< 5$ Hz (near-zero)
\end{itemize}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\textwidth]{figures/thought_individual_analysis.png}
\caption{\textbf{Detailed analysis of single thought reveals circuit completion structure.} 
\textbf{(A)} 3D configuration of Thought 0: Hole center (red star) at origin surrounded by 51 electrons (blue diamond at center, representing circuit completion site) and oxygen molecules (spheres) distributed in 0.15$\times$0.15$\times$0.15 \text{\AA} volume. Color indicates distance from hole (purple=0.04 \text{\AA}, yellow=0.14 \text{\AA}). Most molecules cluster within 0.05 \text{\AA} (purple/blue), with sparse outer shell. Grid shows sub-Ångström coordinate system. Legend identifies hole center and electron. Physical interpretation: Circuit completion requires oxygen molecules to arrange within $\sim$0.1 \text{\AA} of specific configuration—this extreme precision is why each thought is unique. Electron at center represents the "completion event"—the moment circuit closes and consciousness occurs. 
\textbf{(B)} Radial distribution: Histogram shows distance from hole (x-axis: 0.02-0.16 \text{\AA}, n=51 molecules). Peak at 0.10 \text{\AA} (11 counts) with mean=0.10 \text{\AA} (red dashed line) and median=0.10 \text{\AA} (orange dashed line). Range: 0.0-0.2 \text{\AA} (annotated box). Near-Gaussian distribution indicates molecules settle at characteristic distance—the "completion radius" where weak forces balance. Standard deviation $\sim$0.03 Å shows tight clustering, confirming sub-Ångström precision requirement. 
\textbf{(C)} Oscillatory signature: Bar chart shows 30 signature components (x-axis) with values 0.0-0.8 (y-axis). Annotation box: "Dim: 30, Mean: 0.142, Std: 0.195". Components 0-1 dominate (purple/blue bars, $\sim$0.5-0.6), with secondary peak at component 27 (red bar, $\sim$0.8). Most components near zero (green bars, $<$0.2). This sparse signature indicates thought is defined by few dominant weak force configurations—most possible arrangements contribute negligibly. Sparsity explains computational efficiency: brain need only match a few key oscillatory modes, not all $10^6$ possibilities. 
\textbf{(D)} Pairwise distance matrix: 51$\times$51 heatmap shows distances between all oxygen pairs. Color: dark blue (0.00 \text{\AA}, diagonal), yellow (0.25 \text{\AA}, off-diagonal). Annotation box: "Mean: 0.13 \text{\AA}, Min: 0.05 \text{\AA}, Max: 0.26 \text{\AA}". Diagonal is zero (self-distance). Off-diagonal shows banded structure: molecules cluster in groups (blue bands) separated by $\sim$0.15 \text{\AA} gaps (yellow). This reveals hierarchical organization—oxygen molecules form sub-clusters within the ensemble, each sub-cluster completing part of the circuit. Total circuit completion requires all sub-clusters to align simultaneously—explaining why consciousness is all-or-nothing (Section 6.2): partial completion is not experienced.}
\label{fig:thought_individual}
\end{figure}


\textbf{Result}: Conscious frame rate of approximately 40 Hz, matching the theoretical hole-generation rate. REM sleep maintains a similar rate (explaining vivid dream consciousness), while deep sleep suppresses generation (explaining the lack of consciousness). $\checkmark$
...
\textbf{Measurement}: Dream reports analysed for logical violations:
\begin{itemize}
...
\textbf{Time course}: Lucid dreamers report that absurdity increases with dream duration: $\sim$5\% logical violations per minute of dream time, compounding to near-certain absurdity after 10-15 minutes.

\textbf{Result}: Dreams universally show high rates of logical violation, which increase with duration as predicted by the unanchored hole-filling model. $\checkmark$
...
\textbf{Evidence}:
\begin{itemize}
\item \textbf{fMRI studies}: "Remembering" activates the same sensory cortices as perception \cite{mechelli2004price}. $\checkmark$
...
\textbf{Result}: All predictions have been confirmed. Memory behaves as generation, not storage.
...
\textbf{Timing match}: Conscious experience occurs at approximately 40 Hz, aligning with the measured hole-generation rate.
...
\textbf{Sleep}: Slow-wave sleep reduces O$_2$ utilisation → fewer holes → reduced consciousness.
...
\item Photoreceptor responses create specific oscillatory holes in the visual pathway
...
\textbf{Validation}: Propofol binds to Complex I (NADH dehydrogenase) in the electron transport chain \cite{branca2014propofol}. $\checkmark$
...
\textbf{Aging consciousness decline}: Reduced mitochondrial function → decreased O$_2$ utilization → fewer holes → impaired consciousness. Prediction: cognitive decline should correlate with mitochondrial dysfunction more than with amyloid/tau pathology.
...
\item Mobile charge carriers (holes) are generated by substrate movement
...
\textbf{Advantage of our theory}: Provides physical mechanism for $\Phi$ generation (O$_2$ metabolism), not just correlation. Explains why certain brain states have high $\Phi$ (high O$_2$ utilisation) and others do not (anaesthesia disrupts O$_2$ cascades).
