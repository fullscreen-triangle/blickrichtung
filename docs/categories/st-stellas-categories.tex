...
\begin{itemize}
\item \textbf{Global}: Enzyme catalyzes A + B ⇌ C + D (overall reaction)
...
\begin{equation}
\boxed{C_{\text{global}} = \{C_{\text{local},1}, C_{\text{local},2}, \ldots, C_{\text{local},n}\} \quad \text{(trajectory = sequence)}
\end{equation}
