...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{Figure4_Summary.png}
\caption{\textbf{Eight-scale categorical coupling architecture of living systems.} Biological oscillations span 20 orders of magnitude in frequency, naturally clustering into eight hierarchical domains: Scale 1 (Quantum Membrane, $10^{12}$-$10^{15}$ Hz), Scale 2 (Intracellular Circuits, $10^3$-$10^6$ Hz), Scale 3 (Cellular Information, $10^{-1}$-$10^2$ Hz), Scale 4 (Tissue Integration, $10^{-2}$-$10^1$ Hz), Scale 5 (Microbiome Community, $10^{-4}$-$10^{-1}$ Hz), Scale 6 (Organ Coordination, $10^{-5}$-$10^{-2}$ Hz), Scale 7 (Physiological Systems, $10^{-6}$-$10^{-3}$ Hz), and Scale 8 (Allometric Organism, $10^{-8}$-$10^{-5}$ Hz). Living systems maintain coherent coupling across all scales through the coupling matrix $\mathbf{C}_{\text{biological}}$, enabling information flow across 23 orders of magnitude. Life requires $\prod_{i=1}^{8} C_{ii} \times \prod_{i<j} C_{ij}^{w_{ij}} > C_{\text{threshold}}$—when multi-scale coupling breaks down (e.g., cancer as categorical decoupling), disease emerges. Viruses operate only at single-scale completion ($D = 1$), lacking the hierarchical architecture that defines living systems.}
\label{fig:eight_scale_coupling}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{Figure1_Cellular_Metabolism_Overview.png}
\caption{\textbf{Cellular metabolism as a network of coupled categorical explorers.} Metabolic pathways represent interconnected oscillatory components operating at characteristic frequencies. Glycolysis ($\sim10$ enzymes oscillating at 0.1-10 Hz), TCA cycle ($\sim8$ enzymes at 0.01-1 Hz), oxidative phosphorylation ($\sim40$ protein complexes at 1-100 Hz), and biosynthetic pathways form a vast network of $\sim1,500$ metabolic oscillators. Each enzyme exhibits conformational dynamics (ns-ms timescales) representing active categorical exploration. The total categorical completion rate $\dot{C}_{\text{cytoplasm}} = \sum_{i=1}^{N_{\text{osc}}} \omega_i \cdot A_i$ with $N_{\text{osc}} \sim 10^5$ vastly exceeds the information content of the genome's $\sim2 \times 10^4$ static instruction sets. This quantitative asymmetry—$10^5$ active categorical explorers versus $2 \times 10^4$ passive specifications—reveals why the cytoplasm dominates cellular information processing. Viruses lack autonomous metabolic oscillators, possessing only static protein instructions requiring host oscillatory infrastructure for execution.}
\label{fig:cellular_metabolism}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{Figure2_Phase_Locking_Analysis.png}
\caption{\textbf{Phase-locking and frequency entrainment across biological scales.} Living systems exhibit coherent phase relationships between oscillators at different scales, enabling hierarchical information transfer. (A) Phase-locking between metabolic oscillators (Scale 2: glycolytic oscillations at $\sim1$ Hz) and cellular oscillators (Scale 3: cell cycle progression at $\sim10^{-5}$ Hz) demonstrates multi-scale coordination. (B) Frequency entrainment analysis shows $1:N$ coupling ratios between scales, with fast oscillators modulating slow oscillators through amplitude/frequency modulation. (C) Coherence spectrum reveals peaks at characteristic biological frequencies spanning quantum ($10^{15}$ Hz) to circadian ($10^{-5}$ Hz) scales. (D) Coupling strength matrix $\mathbf{C}_{\text{biological}}$ quantifies inter-scale communication: strong diagonal elements (within-scale coupling) and significant off-diagonal elements (cross-scale coupling) maintain organismal coherence. Loss of phase-locking (e.g., cancer, aging, circadian disruption) represents breakdown of categorical coupling. Viruses, operating at single-scale completion, exhibit no multi-scale phase relationships and cannot maintain temporal coherence characteristic of living systems.}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{three_tier_architecture.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{hierarchy.pdf}
\caption{\textbf{Hierarchical categorical depth ladder from non-living to complex life.} Categorical depth $D$ increases from simple chemical systems ($D = 0$) through the critical life threshold ($D = 3$) to complex multicellular organisms ($D = 7$). $D = 0$ (Prebiotic chemistry): Individual reactions reaching equilibrium, no hierarchical organization, $\dot{C} \to 0$. $D = 1$ (Viruses, autocatalytic cycles): Single categorical completion requiring external machinery; viruses hijack host infrastructure, lacking autonomous oscillatory components ($N_{\text{osc}} = 0$). $D = 2$ (Protocells): Metabolism + genetic replication but no organizational closure; systems drift and fail within 50-100 generations due to lack of synchronized replication-division coupling. \textbf{$D = 3$ (Minimal life threshold)}: Integration of molecular transformations, catalyst regeneration, and organizational replication creates closed hierarchical loop enabling sustained autonomous function ($\lim_{t \to \infty} \dot{C}(t) = \dot{C}_0 > 0$); represents topological phase transition with spontaneous breaking of temporal reversibility symmetry. $D = 4$ (Bacteria, Archaea): Addition of regulatory level (quorum sensing, stress response) provides environmental robustness. $D = 5$ (Protists, simple eukaryotes): Organellar level (mitochondria, nuclei) adds compartmentalization. $D = 6$ (Multicellular fungi, plants): Tissue differentiation enables specialized functions. $D = 7$ (Complex animals): Organ systems and neural integration; highest known categorical depth. The $D = 2 \to 3$ transition exhibits critical phenomena: order parameter discontinuity ($\Psi$ jumps from 0 to $\Psi_0 > 0$), diverging correlation length ($\xi \sim |D-3|^{-\nu}$), and symmetry breaking (temporal irreversibility emerges). This establishes life as a fundamental phase transition in categorical completion space, with the hierarchical ladder providing a quantitative framework for classifying all biological and prebiotic systems.}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{Figure_Statistical_Validation.pdf}
\caption{\textbf{Statistical validation of categorical richness distinguishing viral and host proteins.} (A) Distribution comparison shows High-R proteins (viral-like, red) exhibit significantly higher categorical similarity to host proteins than Low-R proteins (blue), both differing from random expectation (gray). High-R mean: 0.772; Low-R mean: 0.598; $p < 7.25 \times 10^{-25}$. (B) Statistical tests confirm highly significant differences with large effect size (Cohen's $d = 1.61$). (C) Q-Q plot validates normal distribution (Shapiro-Wilk: $W = 0.943$, $p = 0.012$). (D) Bootstrap confidence intervals (95\%) show robust separation: High-R [0.740, 0.805], Low-R [0.555, 0.641]. (E) Power analysis confirms well-powered study (100\% power at $n=54$). These results demonstrate that viral proteins evolved high categorical richness ($R > 10^6$) to occupy cellular categorical space, enabling immune evasion during synthesis.}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{Figure1_Viral_Mimicry_Heatmap.pdf}
\caption{\textbf{Viral proteins evade detection through categorical camouflage via high-R molecular mimicry.} (A) Heatmap showing categorical similarity between viral proteins (rows) and high-R host signaling proteins (columns). High-R viral proteins (HIV Tat, Nef, Vpr; HPV E6, E7; EBV LMP1, EBNA1; HCV Core) exhibit 0.61-0.94 similarity to host proteins (red), enabling evasion of cellular quality control. Low-R proteins (HIV p24, Histone H3, Actin) show only 0.27-0.45 similarity (blue). (B) High-R proteins exhibit significantly higher categorical similarity (0.82 $\pm$ 0.05) than Low-R proteins (0.36 $\pm$ 0.08), $p < 6.25 \times 10^{-25}$. (C) Validation pie chart: 88.9\% (8/9) of predicted high-R viral targets validated experimentally. (D) Target specificity: High-R viral proteins preferentially target p53, NF-$\kappa$B, PKC, STAT3, and Src—exactly matching simulation predictions (78\%, 78\%, 67\% targeting rates). (E) Categorical similarity distributions confirm distinct populations: High-R proteins cluster at 0.8-0.9 (mean: 0.820), Low-R at 0.3-0.4 (mean: 0.360), far exceeding random baseline (0.25). These results demonstrate that viruses evade immune detection during synthesis by occupying host categorical space through high-R mimicry, with detection occurring only upon categorical completion (viral assembly).}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{figure3_network_depth.pdf}
...
\textbf{Quantifying autonomy}: We have defined autonomy qualitatively (presence/absence of external machinery). A quantitative metric would strengthen the framework.

\textbf{Borderline cases}: Systems at $D = 3$ (minimal cells, early life) are at the threshold. Additional criteria may be needed for these edge cases.

\textbf{Cognitive levels}: Whether consciousness constitutes an additional categorical level ($\mathcal{L}_6$ or $\mathcal{L}_7$) in complex animals requires further analysis.

\textbf{Experimental validation}: Direct measurement of categorical depth in living systems requires new experimental approaches, potentially involving information-theoretic or network-based analyses.

\textbf{Computational life}: Whether computational systems (e.g., artificial intelligence, digital organisms) can achieve hierarchical categorical completion remains an open question.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{Figure1_Oscillatory_State_Space.png}
