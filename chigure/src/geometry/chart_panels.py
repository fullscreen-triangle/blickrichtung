...
def figure_1_ion_tunneling(save_path='figure_1_ion_tunneling.png'):
    ...
    with open('consciousness_quantum_validation/experiment_1_ion_tunneling.json', 'r') as f:
        data = json.load(f)
    ...
    h_magnitude = eval(data['ion_quantum_data']['H+']['field_magnitude'])
    h_phase = eval(data['ion_quantum_data']['H+']['field_phase'])
...
def figure_2_coherence_fields(save_path='figure_2_coherence_fields.png'):
    ...
    ax1 = fig.add_subplot(gs[0, 0])
    region = 'region_0'
    magnitude = eval(data['region_data'][region]['magnitude'])
    ...
    ax2 = fig.add_subplot(gs[0, 1])
    phase = eval(data['region_data'][region]['phase'])
    ...
    for region in regions:
        h_field = eval(data['region_data'][region]['ion_contributions']['H+'])
    ...
    ion_magnitudes = []
    for ion in ions:
        ion_field = eval(data['region_data'][region]['ion_contributions'][ion])
    ...
    region = 'region_0'
    regional_field = eval(data['region_data'][region]['regional_field'])
    ...
    # Add unit circle
    circle = Circle((0, 0), np.mean(np.abs(field_sample)), 
...
def figure_3_decoherence_resistance(save_path='figure_3_decoherence_resistance.png'):
    ...
    for idx, color, label in zip(noise_levels, colors_dist, labels_dist):
        magnitude = eval(thermal_results[idx]['field_data']['magnitude'])
    ...
    for idx, color, label in zip(noise_levels, colors_dist, labels_dist):
        phase = eval(thermal_results[idx]['field_data']['phase'])
...
def figure_4_state_transitions(save_path='figure_4_state_transitions.png'):
    ...
    with open('consciousness_quantum_validation/experiment_5_state_transitions.json', 'r') as f:
        data = json.load(f)
    ...
    quantum_field = eval(data['quantum_field_trace'])
